...
!! added by ADAPT
      ...
      integer :: adapt_watb_hru(999)
      integer :: adapt_othr_hru(999)
      integer :: adapt_i
!!
...
!! added by ADAPT
      adapt_h = adaptfvolh(res_vol(jres), jres) !! reservoir height
      ressa = adaptfsurh(res_vol(jres), adapt_h, jres)   !! reservoir surface

      adapt_qmax = adaptfmaxd(adapt_h, jres)    !! maximum spillage from curve and monthly value

      !! Accounting for reservoir surface changes in HRU computations
      adapt_sub_area = sub_fr_adapt(res_sub(jres))*da_ha
      ressa = max(ressa, sub_fr_adapt(res_sub(jres))*da_ha-1.)
      sub_fr(res_sub(jres)) = (adapt_sub_area-min(ressa,adapt_sub_area))&
     &  /da_ha
      sub_km(res_sub(jres)) = sub_fr(res_sub(jres))*da_ha/100.

      do adapt_hru = 1, nhru
        if (hru_sub(adapt_hru) == res_sub(jres)) then
          !! For each HRU within the subbasin
          if (idplt(adapt_hru) /= 0) then
            if (bio_e(idplt(adapt_hru)) <= 1.e-6) then
              !! A WATB HRU within the reservoir's subbasin
              adapt_watb_cntr = adapt_watb_cntr + 1
              adapt_watb_hru(adapt_watb_cntr) = adapt_hru
              adapt_hru_area = adapt_hru_area + hru_fr_adapt(adapt_hru)*&
     &            adapt_sub_area/100.
            else
              !! A normal HRU within the reservoir's subbasin
              adapt_othr_cntr = adapt_othr_cntr + 1
              adapt_othr_hru(adapt_othr_cntr) = adapt_hru
            end if
            else
              !! A normal HRU within the reservoir's subbasin
              adapt_othr_cntr = adapt_othr_cntr + 1
      ...
      do adapt_i = 1, adapt_watb_cntr
        !! correction of WATB HRUs
        hru_fr(adapt_watb_hru(adapt_i)) = hru_fr_adapt(                 &
     &      adapt_watb_hru(adapt_i))*max(0.,(adapt_hru_area-ressa/100.  &
     &      )/adapt_hru_area)*sub_fr_adapt(res_sub(jres))/sub_fr(       &
     &      res_sub(jres))
        hru_ha(adapt_watb_hru(adapt_i)) = hru_fr(adapt_watb_hru(adapt_i)&
     &      )*sub_fr(res_sub(jres))*da_ha
        hru_km(adapt_watb_hru(adapt_i)) = hru_ha(adapt_watb_hru(adapt_i)&
     &      )/100.
     !!   print *, "WATB ", hru_fr(adapt_watb_hru(adapt_i)),              &
     !!&      hru_fr_adapt(adapt_watb_hru(adapt_i)), hru_ha(adapt_watb_hru&
     !!&      (adapt_i))
      end do

      do adapt_i = 1, adapt_othr_cntr
        !! correction of non WATB HRUs
        if (ressa/100. - adapt_hru_area>0.) then
          !! changes beyond WATB HRUs
          hru_fr(adapt_othr_hru(adapt_i)) = hru_fr_adapt(               &
     &        adapt_othr_hru(adapt_i))*max(0.,1-(ressa/100.-            &
     &        adapt_hru_area)/(adapt_sub_area/100.-adapt_hru_area))*    &
     &        sub_fr_adapt(res_sub(jres))/sub_fr(res_sub(jres))
        else
          !! no changes beyond WATB HRUs
          hru_fr(adapt_othr_hru(adapt_i)) = hru_fr_adapt(               &
     &        adapt_othr_hru(adapt_i))*sub_fr_adapt(res_sub(jres))/     &
     &        sub_fr(res_sub(jres))
        endif
        hru_ha(adapt_othr_hru(adapt_i)) = hru_fr(adapt_othr_hru(adapt_i)&
     &      )*sub_fr(res_sub(jres))*da_ha
        hru_km(adapt_othr_hru(adapt_i)) = hru_ha(adapt_othr_hru(adapt_i)&
     &      )/100.
     !!   print *, "OTHR ", hru_fr(adapt_othr_hru(adapt_i)),              &
     !!&      hru_fr_adapt(adapt_othr_hru(adapt_i)), hru_ha(adapt_othr_hru&
     !!&      (adapt_i))
      enddo
      subfr_nowtr(res_sub(jres)) = (sub_fr_adapt(res_sub(jres))*        &
     &    da_ha/100.-adapt_hru_area)/(sub_km(res_sub(jres)))

      !!print *, "jres ", jres
      !!print *, "res_sub(jres) ", res_sub(jres)
      !!print *, "adapt_h", adapt_h
      !!print *, "res_vol(jres)", res_vol(jres)
      !!print *, "sub_km ", sub_km(res_sub(jres))
      !!print *, "adapt_hru_area ", adapt_hru_area
      !!print *, "new_sub_km", sub_fr(res_sub(jres))*da_ha/100.
      !!print *, "da_ha ", da_ha
      !!print *, "ressa ", ressa
      !!print *, "subfr_nowtr(res_sub(jres)) ", subfr_nowtr(res_sub(jres))
!!
...
!! if reservoir volume is greater than zero
     ...
     &              **adapt_beta(jres)
          ...
          case (7)
            ...
            flw = 0.
            resflwo=-999
            if (adapt_outflow(jres)==1) then
              !! There is a daily measurement
              read (350+jres, 5000) flw
              resflwo = 86400. * flw
            end if
