...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !! added by ADAPT
      character (len=13) :: hvcurve, mdcurve, hacurve
      integer :: adapt_counter, adapt_type, adapt_ffound
      !!
      integer :: eof, mon, j
...
!!    read in data from .res file
      do
        ...
        if (eof < 0) exit
        !!!! added by ADAPT
        read (105,*,iostat=eof) adapt_res_eh(i)
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_res_ph(i)
        if (eof < 0) exit
        read (105,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (105,1000,iostat=eof) titldum
        ...
        read (105,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_b0(i)
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_b1(i)
        if (eof < 0) exit
        read (105,1100,iostat=eof) hvcurve
        if (eof < 0) exit
        read (105,1100,iostat=eof) mdcurve
        if (eof < 0) exit
        read (105,1100,iostat=eof) hacurve
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_alpha(i)
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_h0(i)
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_beta(i)
        if (eof < 0) exit
        read (105,*,iostat=eof) adapt_k(i)
        if (eof < 0) exit
        !!!!
        exit
      end do
...
!!    open daily reservoir outflow file 
      ...
      if (iresco(i) == 7) then
        call caps(resdayo)
        open (350+i,file=resdayo, iostat=adapt_ffound, action="read")
        if (adapt_ffound .ne. 0) then
          !! There is no daily series for this reservoir
          adapt_outflow(i)=0
        else
          !! There is daily series for this reservoir
          adapt_outflow(i)=1
          read (350+i,1000) titldum
        end if
        !!print *, i, "  ", adapt_outflow(i)
      end if

      if (hvcurve/="") then
        call caps(hvcurve)
        open (101,file=hvcurve)
        adapt_counter = 1
        do
            read (101,9101,iostat=eof) (adapt_hv(i,adapt_counter,       &
     &          adapt_type), adapt_type = 1, 2)
            if (eof < 0) exit
            adapt_counter = adapt_counter+1
        end do
        do j=1, adapt_counter-1
            !!print *, adapt_hv(i,j,1), "     ", adapt_hv(i,j,2)
            adapt_hv(i,j,2) = adapt_hv(i,j,2) * 10000.  !! 10**4 m**3 => m**3
        end do
        close (101)
      end if
      ...
      if (mdcurve/="") then
        call caps(mdcurve)
        open (101,file=mdcurve)
        adapt_counter = 1
        do
            read (101,9101,iostat=eof) (adapt_md(i,adapt_counter,       &
     &          adapt_type), adapt_type = 1, 2)
            if (eof < 0) exit
            adapt_counter=adapt_counter+1
        end do
        do j=1, adapt_counter-1
            !!print *, adapt_md(i,j,1), "     ", adapt_md(i,j,2)
            adapt_md(i,j,2) = adapt_md(i,j,2) * 86400.  !! m**3/s => m**3/day
        end do
        close (101)
      end if
      ...
      if (hacurve/="") then
        call caps(hacurve)
        open (101,file=hacurve)
        adapt_counter = 1
        do
            read (101,9101,iostat=eof) (adapt_ha(i,adapt_counter,          &
     &          adapt_type), adapt_type = 1, 2)
            if (eof < 0) exit
            adapt_counter=adapt_counter+1
        end do
        do j=1, adapt_counter-1
            !!print *, adapt_ha(i,j,1), "     ", adapt_ha(i,j,2)
            adapt_ha(i,j,1) = adapt_ha(i,j,1)
        end do
        close (101)
      end if
      !!
...
!! read in monthly release data
 ...
 1000 format (a80)
 1100 format (a13)
 !! added by ADAPT for .cur files reading
 9101 format(F8.0,T9,F16.0)
 !!
