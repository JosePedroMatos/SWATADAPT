...
! msub = max number of subbasins
      ...
      !! added by ADAPT
      real, dimension (:), allocatable :: sub_fr_adapt
      !!
...
! mres = max number of reservoirs
      ...
      !! added by ADAPT for reservoir and wetland routing
      real, dimension (:), allocatable :: adapt_res_eh, adapt_res_ph
      real, dimension (:), allocatable :: adapt_b0, adapt_b1
      real, dimension (:), allocatable :: adapt_alpha, adapt_h0
      real, dimension (:), allocatable :: adapt_beta, adapt_k

      real, dimension (:,:,:), allocatable :: adapt_hv, adapt_md
      real, dimension (:,:,:), allocatable :: adapt_ha

      integer, dimension (:), allocatable :: adapt_outflow
      !!
...
! mhru = maximum number of hydrologic response units
      ...
      !! added by ADAPT
      real, dimension (:), allocatable :: hru_fr_adapt
      !!
...
!! By Zhang for C/N cycling
	...
	real, dimension(:), allocatable :: sedc_m, surfqc_m, latc_m,
     &  percc_m, foc_m, NPPC_m, rsdc_m, grainc_m, stoverc_m, emitc_m,
     &  soc_m, rspc_m
...
!! ADAPT custom variables
      ! code for text output of files (.rch, .sub, .hru files only)
      integer :: printTxt
